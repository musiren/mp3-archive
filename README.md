...
- **재생목록 / 큐**: 탭하면 큐에 추가+재생, 재생 탭에 큐 표시(재생중 강조),
  ...
  반복 모드(전체한번만·한곡반복·전체반복)와 **셔플 토글**(독립 아이콘),
  곡 끝나면 자동 다음곡, 빈 상태에서 재생 버튼은 첫 곡(셔플 시 셔플 순서의
  첫 곡)부터 재생, `.list` 저장/불러오기(없는 파일 건너뜀)/비우기, 트리 폴더
  롱프레스로 폴더 전체 추가
- **셔플 토글**: 트랜스포트 바 왼쪽 아이콘으로 셔플을 독립적으로 켜고 끔.
  켜면 새 시드로 즉시 순서를 재편하고, 끄면 현재 곡부터 설정된 반복 모드
  순서로 재개. 오른쪽 반복 버튼은 셔플과 무관하게 전체한번만 → 한곡반복 →
  전체반복을 순환
...
- **상태 저장/복원**: 앱을 종료해도 다음 실행 때 그대로 — 재생목록(`.list`
  ...
  셔플 on/off·반복 모드·셔플 순서, 앨범아트 표시 여부, 목록 탭 보기 모드,
  테마, 마지막으로 사용한 라이브러리 DB
