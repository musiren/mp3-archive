...
- **스캔 / 검색**: 인앱 파일 관리자로 폴더 선택 → 라이브러리 교체(이전 스캔
  내용을 비우고 선택한 폴더만 표시) / 새로고침으로 현재 폴더 전체 재스캔;
  파일명·태그 실시간 검색
...
- **재생목록 / 큐**: 탭하면 큐에 추가+재생, 재생 탭에 큐 표시(재생중 강조),
  큐 곡 롱프레스로 재생/제거, 이전/다음, 재생모드(순차·한곡반복·전체반복·셔플),
  곡 끝나면 자동 다음곡, 빈 상태에서 재생 버튼은 첫 곡(셔플 시 무작위)부터 재생,
...
- **홈 화면 위젯**: 크기 조절 가능한 앱 위젯이 앨범아트 위에 현재 곡 제목·
  아티스트(태그 없으면 파일명)와 이전/재생·일시정지/다음 버튼을 표시. 위젯
  본문을 탭하면 앱이 열리고, 반투명 스크림으로 밝은 앨범아트 위에서도 글씨가
  잘 보이며, 버튼은 알림·잠금화면과 같은 서비스를 제어
- **로딩 화면**: 흰 배경 + 앱 아이콘 + 버전
