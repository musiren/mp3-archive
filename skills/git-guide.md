...
> 아래 두 규칙은 예외 없이 항상 적용된다.
> 사용자가 명시적으로 허락하지 않는 한 절대 어기지 않는다.

### 1. 푸시 전 반드시 사용자에게 확인한다

`git push` 를 실행하기 **전에** 반드시 사용자에게 물어본다.
사용자가 "푸시해줘" 라고 직접 요청한 경우에만 푸시한다.
자동으로, 또는 작업 흐름의 일부로 묵시적으로 푸시하지 않는다.

### 2. UI 파일이 변경되면 반드시 ui-preview.jpg 를 업데이트한다

`src/main_window.ui` 또는 `src/main_window.py` 가 변경된 경우,
커밋 및 푸시 **전에** `docs/ui-preview.jpg` 를 재생성하고 함께 커밋해야 한다.
프리뷰 이미지 업데이트 없이 UI 변경을 푸시하지 않는다.
